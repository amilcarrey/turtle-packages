...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@turtledev/ui";
import { cn } from "../../lib/utils";

interface MenuBarProps<T extends string> {
  selectedValue: T;
  ...
  return (
    <>
      <div
        ref={containerRef}
        className={cn(
          "relative hidden h-10 w-full gap-2.5 rounded-full border border-white/10 bg-[var(--color-surface-primary)] font-medium sm:flex",
          className,
        )}
      >
        <div
          ref={indicatorRef}
          className="absolute bottom-0 h-full origin-left rounded-full bg-[var(--color-surface-secondary)] transition-all duration-300"
        />
